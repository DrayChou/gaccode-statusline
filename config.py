...
class ConfigManager:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        if self._should_reload_config():
            if self.unified_config_file.exists():
                ...
                if config is None:
                    log_message(
                        "config",
                        "ERROR",
                        f"Failed to load config from {self.unified_config_file}",
                    )
            ...
            self._last_modified = (
                self.unified_config_file.stat().st_mtime
                if self.unified_config_file.exists()
                else None
            )
    ...
    def save_config(self, config: Dict[str, Any]) -> bool:
        ...
        if safe_json_write(self.unified_config_file, safe_config):
            ...
            log_message(
                "config", "INFO", f"Configuration saved to {self.unified_config_file}"
            )
            return True
        else:
            log_message(
                "config",
                "ERROR",
                f"Failed to save configuration to {self.unified_config_file}",
            )
            return False
    ...
    def get_platform_api_key(self, platform_id: str) -> Optional[str]:
        ...
        config_key = (
            ...
            or platform.get("auth_token")
            or platform.get("login_token")
        )
        if config_key:
            log_message(
                "config",
    ...
    def set_default_platform(self, platform_id: str) -> bool:
        ...
        if not platform_config.get("enabled", False):
            log_message(
                "config", "WARNING", f"Platform {resolved_platform} is disabled"
            )
    ...
    def get_effective_platform(self, session_id: Optional[str] = None) -> str:
        ...
        if session_id:
            try:
                ...
                mapping_file = (
                    self.project_dir / "data" / "cache" / "session-mappings.json"
                )
