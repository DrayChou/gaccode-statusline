...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Input Validation and Security Testing")
    parser.add_argument("--test-path", metavar="PATH", help="Test path security validation")
    parser.add_argument("--test-filename", metavar="FILENAME", help="Test filename validation")
    parser.add_argument("--test-api-key", nargs=2, metavar=("PLATFORM", "KEY"), help="Test API key validation")
    parser.add_argument("--test-url", metavar="URL", help="Test URL validation")
    parser.add_argument("--test-json", metavar="JSON", help="Test JSON validation")
    parser.add_argument("--sanitize", metavar="TEXT", help="Test string sanitization")
    ...
    try:
        if args.test_path:
            result = validator.validate_path_security(args.test_path)
            print(f"Path validation passed: {result}")
        
        elif args.test_filename:
            result = validator.validate_filename(args.test_filename)
            print(f"Filename validation passed: {result}")
        
        elif args.test_api_key:
            platform, key = args.test_api_key
            is_valid, message = validator.validate_api_key(key, platform)
            print(f"API key validation: {'PASSED' if is_valid else 'FAILED'}")
            print(f"Message: {message}")
        
        elif args.test_url:
            result = validator.validate_url(args.test_url)
            print(f"URL validation passed: {result}")
        
        elif args.test_json:
            result = validator.validate_json_data(args.test_json)
            print(f"JSON validation passed: {type(result)} with {len(result) if hasattr(result, '__len__') else 'N/A'} items")
        
        elif args.sanitize:
            result = validator.sanitize_string(args.sanitize)
            print(f"Original: {args.sanitize}")
            ...
            test_cases = [
                ("路径验证", lambda: validator.validate_path_security("./test.json")),
                ("文件名验证", lambda: validator.validate_filename("config.json")),
                ("API密钥验证", lambda: validator.validate_api_key("sk-test1234567890", "openai")),
                ("URL验证", lambda: validator.validate_url("https://api.example.com")),
                ("JSON验证", lambda: validator.validate_json_data('{"test": "value"}')),
                ("字符串清理", lambda: validator.sanitize_string("  test\n\tstring  "))
            ]
