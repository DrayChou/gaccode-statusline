...
def interactive_config():
    ...
    for key, desc in options:
        current = config[key]
        while True:
            response = input(f"{desc} [当前: {'是' if current else '否'}] (y/n/回车保持不变): ").strip().lower()
            if response == '':
                break
    ...
    if config['show_directory']:
        ...
        while True:
            response = input(f"显示完整目录路径 [当前: {'是' if current else '否'}] (y/n/回车保持不变): ").strip().lower()
            if response == '':
                break
            elif response in ['y', 'yes', '是']:
    ...
    while True:
        response = input(f"显示布局 [当前: {current}] (single/multi/回车保持不变): ").strip().lower()
        if response == '':
            break
...
def main():
    ...
    parser.add_argument("--set", nargs=2, metavar=("KEY", "VALUE"), help="设置单个配置项")
