...
class CacheManager:
    ...
    def clear_namespace(self, namespace: str) -> int:
        ...
        log_message(
            "cache",
            "INFO",
            f"Cleared {cleared_count} cache entries from namespace '{namespace}'",
        )
    ...
    def cleanup_expired(self) -> int:
        ...
        if cleaned_count > 0:
            log_message(
                "cache", "INFO", f"Cleaned up {cleaned_count} expired cache entries"
            )
        ...
        def decorator(func: Callable[..., T]) -> Callable[..., T]:
            def wrapper(*args, **kwargs) -> T:
                ...
                log_message(
                    "cache",
                    "DEBUG",
                    f"Cache miss for {namespace}:{key}, executing function",
                )
...
# 便捷函数
def cache_get(
    namespace: str, key: str, params: Optional[Dict[str, Any]] = None
) -> Optional[Any]:
    """便捷的缓存获取函数"""
